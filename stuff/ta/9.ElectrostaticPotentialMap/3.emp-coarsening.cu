...
{
  const int i0 = blockIdx.x * blockDim.x * COARSEN_FACTOR + threadIdx.x * COARSEN_FACTOR;
  const int j = blockIdx.y * blockDim.y + threadIdx.y;

  const float y = GRID_SPACING * (float)j;
  const float x0 = GRID_SPACING * (float)i0;
...
#pragma unroll(COARSEN_FACTOR)
    ...
    const int grid_offset = GRID_DIM * GRID_DIM * t + GRID_DIM * j + i0;
#pragma unroll(COARSEN_FACTOR)
    for (int i_off = 0; i_off < COARSEN_FACTOR; ++i_off)
      energygrid[grid_offset + i_off] += energy[i_off];
