...
void cenergy(float *energygrid, const float *atoms)
{
  for (int t = 0; t < GRID_DIM; t++)
  {
    ...
    const int grid_slice_offset = GRID_DIM * GRID_DIM * t;
...
{
  const int i = blockIdx.x * blockDim.x * COARSEN_FACTOR + threadIdx.x;
  const int j = blockIdx.y * blockDim.y + threadIdx.y;

  const float y = GRID_SPACING * (float)j;
  const float x0 = GRID_SPACING * (float)i;

  for (int t = 0; t < GRID_DIM; t++)
  {
    const float z = GRID_SPACING * (float)t;
    // TODO
    float energy[COARSEN_FACTOR];
    memset(energy, 0.0f, sizeof(energy));
    // calculate potential contribution of each atom
    for (int n = 0; n < NUM_ATOMS * ATOM_STRIDE; n += ATOM_STRIDE)
    {
      const float dy = y - d_atoms[n + 1];
      const float dz = z - d_atoms[n + 2];
      const float charge = d_atoms[n + 3];
      const float dy2dz2 = dy * dy + dz * dz;

      float dx = x0 - d_atoms[n];
#pragma unroll(COARSEN_FACTOR)
      for (int i_off = 0; i_off < COARSEN_FACTOR; ++i_off)
      {
        energy[i_off] += charge / sqrtf(dx * dx + dy2dz2);
        dx += GRID_SPACING * blockDim.x;
      }
    }

    const int grid_offset = GRID_DIM * GRID_DIM * t + GRID_DIM * j + i;
#pragma unroll(COARSEN_FACTOR)
    for (int i_off = 0; i_off < COARSEN_FACTOR; ++i_off)
      energygrid[grid_offset + i_off * blockDim.x] += energy[i_off];
...
int main()
{
  ...
  const dim3 numBlocks((GRID_DIM + (BLOCK_DIM * COARSEN_FACTOR) - 1) / (BLOCK_DIM * COARSEN_FACTOR),
                       (GRID_DIM + BLOCK_DIM - 1) / BLOCK_DIM);
    ...
    for (int j = 0; j < GRID_DIM; j++)
    {
      for (int i = 0; i < GRID_DIM; i++)
      {
                     ...
                     h_energygrid[GRID_DIM * GRID_DIM * t + GRID_DIM * j + i]) > 1e-3)
        {
          ...
          std::cout << j << std::endl;
          std::cout << i << std::endl;
          std::cout << energygrid[GRID_DIM * GRID_DIM * t + GRID_DIM * j + i] << std::endl;
          std::cout << h_energygrid[GRID_DIM * GRID_DIM * t + GRID_DIM * j + i] << std::endl;
          return EXIT_FAILURE;
        }
      }
    }
