...
void __global__ cenergy_gpu(float *energygrid)
{
  ...
  const int grid_xy_offset = GRID_DIM * j + i;
  for (int t = 0; t < GRID_DIM; t++)
  {
    const float z = GRID_SPACING * (float)t;
    float energy = 0.0f;
    // TODO
    const int grid_offset = GRID_DIM * GRID_DIM * t + grid_xy_offset;
    // calculate potential contribution of each atom
    for (int n = 0; n < NUM_ATOMS * ATOM_STRIDE; n += ATOM_STRIDE)
    {
...
int main()
{
  ...
  const dim3 numBlocks((GRID_DIM + BLOCK_DIM - 1) / BLOCK_DIM, (GRID_DIM + BLOCK_DIM - 1) / BLOCK_DIM);
