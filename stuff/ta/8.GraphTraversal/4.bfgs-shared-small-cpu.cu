...
void insertIntoFrontier(const int val, int *frontier, int &frontier_size)
{
...
void BFS_iterate_sequential(int **previousFrontier,
                            int &previousFrontierSize,
                            int **currentFrontier,
...
void BFS_sequential(const int source, const int *rowPointers, const int *destinations, int *distances)
{
  ...
  {
    BFS_iterate_sequential(&previousFrontier,
                           previousFrontierSize,
                           &currentFrontier,
                           currentFrontierSize,
                           rowPointers,
                           destinations,
  ...
  free(frontiers_host[0]);
  free(frontiers_host[1]);
  free(frontiers_host);
}
...
__global__ void BFS_Bqueue_kernel(const int *previousFrontier,
                                  ...
                                  int *visited)
{
  __shared__ int sharedCurrentFrontier[MAX_FRONTIER_SIZE];
  ...
  {
    // TODO computation of this frontier
    ...
    for (int i = rowPointers[currentVertex]; i < rowPointers[currentVertex + 1]; ++i)
    {
      ...
      if (!alreadyVisited)
      {
        ...
        if (sharedQueueIndex < MAX_FRONTIER_SIZE)
        {
          sharedCurrentFrontier[sharedQueueIndex] = destinations[i];
        }
        else
        {
          sharedCurrentFrontierSize = MAX_FRONTIER_SIZE;
              ...
              const int num_vals)
{
  ...
  for (int i = 0; i < 2; i++)
    frontiers_host[i] = (int *)calloc(MAX_FRONTIER_SIZE, sizeof(int));
  int *hCurrentFrontier = frontiers_host[0];
  int hCurrentFrontierSize = 0;
  int *hPreviousFrontier = frontiers_host[1];
  int hPreviousFrontierSize = 0;
  int *hVisited = (int *)calloc(num_vals, sizeof(int *));
  insertIntoFrontier(source, hPreviousFrontier, hPreviousFrontierSize);
  distances[source] = 0;
  *(hVisited + source) = 1;

  // Allocate frontiers for GPU
  ...
  // Make the assumption that we start with the CPU at least at the first iteration
  while (hPreviousFrontierSize > 0)
  {
    ...
    if (numBlocks < NUM_BLOCKS_THRESHOLD)
    {
      ...
      {
        // TODO Update data on host
        CHECK(cudaMemcpy(hCurrentFrontier, dCurrentFrontier, sizeof(int) * MAX_FRONTIER_SIZE, cudaMemcpyDeviceToHost));
        CHECK(cudaMemcpy(hPreviousFrontier, dPreviousFrontier, sizeof(int) * MAX_FRONTIER_SIZE, cudaMemcpyDeviceToHost));
        CHECK(cudaMemcpy(hVisited, dVisited, sizeof(int) * num_vals, cudaMemcpyDeviceToHost));
        CHECK(cudaMemcpy(distances, dDistances, sizeof(int) * num_vals, cudaMemcpyDeviceToHost));
        CHECK(cudaMemcpy(&hCurrentFrontierSize, dCurrentFrontierSize, sizeof(int), cudaMemcpyDeviceToHost));
        CHECK(cudaMemcpy(&hPreviousFrontierSize, dPreviousFrontierSize, sizeof(int), cudaMemcpyDeviceToHost));
        currentDataOnDevice = false;
      }
      BFS_iterate_sequential(&hPreviousFrontier,
                             hPreviousFrontierSize,
                             &hCurrentFrontier,
                             hCurrentFrontierSize,
                             rowPointers,
                             destinations,
                             distances);
      for (int i = 0; i < hPreviousFrontierSize; i++)
        hVisited[hPreviousFrontier[i]] = 1;
    }
    else
    {
      if (!currentDataOnDevice)
      {
        // TODO update data on device
        CHECK(cudaMemcpy(dCurrentFrontier, hCurrentFrontier, sizeof(int) * MAX_FRONTIER_SIZE, cudaMemcpyHostToDevice));
        CHECK(cudaMemcpy(dPreviousFrontier, hPreviousFrontier, sizeof(int) * MAX_FRONTIER_SIZE, cudaMemcpyHostToDevice));
        CHECK(cudaMemcpy(dVisited, hVisited, sizeof(int) * num_vals, cudaMemcpyHostToDevice));
        CHECK(cudaMemcpy(dDistances, distances, sizeof(int) * num_vals, cudaMemcpyHostToDevice));
        CHECK(cudaMemcpy(dCurrentFrontierSize, &hCurrentFrontierSize, sizeof(int), cudaMemcpyHostToDevice));
