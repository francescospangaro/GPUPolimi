                                 ...
                                 const unsigned int length)
{
  ...
  unsigned int *block_histogram = &histogram[blockIdx.x * BIN_NUM];
  // Write to this blocks' histogram
  for (int i = tid; i < length; i += stride)
  {
    int alphabet_position = data[i] - FIRST_CHAR;
    if (alphabet_position >= 0 && alphabet_position < ALPHABET_SIZE)
      atomicAdd(&block_histogram[alphabet_position / CHAR_PER_BIN], 1);
  }
...
#pragma unroll
    ...
    {
      const unsigned int binValue = block_histogram[binIdx];
      if (binValue > 0)
