...
bool arrayEquals(int *a1, int *a2, int dim)
{
    for (int i = 0; i < dim; ++i)
        if (a1[i] != a2[i])
            return false;
...
int main(int argc, char **argv)
{
    ...
    printf("Kernel 1: %s\n", arrayEquals(C, C_h, sa * sb) ? "OK" : "KO");

    CHECK(cudaMemset(C_d, 0, sa * sb * sizeof(int)));
    compareSharedMemKernel<<<numOfBlocks, threadsPerBlock>>>(A1_d, B1_d, sa, sb, C_d);
    CHECK_KERNELCALL();
    CHECK(cudaMemcpy(C_h, C_d, sizeof(int) * sa * sb, cudaMemcpyDeviceToHost));
    printf("Kernel 2: %s\n", arrayEquals(C, C_h, sa * sb) ? "OK" : "KO");
