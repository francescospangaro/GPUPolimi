...
void initData(float *surf, int nx, int ny)
{
...
void compute(float **surfPtr, float **surf1Ptr, int nx, int ny, int nsteps)
{
  float *surf = *surfPtr;
  float *surf1 = *surf1Ptr;
...
__global__ void initKernel(float *const __restrict__ surf,
                           int const nx,
                           int const ny)
{
...
__global__ void computeStepKernel(const float *const __restrict__ surf,
...
bool arrayEqualsMoreOrLess(float *a1, float *a2, int size)
{
  for (int i = 0; i < size; ++i)
    if (abs(a1[i] - a2[i]) > 0.001)
      return false;
  return true;
}
...
int main(int argc, char **argv)
{
  ...
  initKernel<<<numOfBlocks, threadsPerBlock>>>(surf_d, nx, ny);
  CHECK_KERNELCALL();

  for (int n = 0; n < nsteps; ++n)
  {
    computeStepKernel<<<numOfBlocks, threadsPerBlock>>>(surf_d, surf1_d, nx, ny);
    CHECK_KERNELCALL();
  ...
  CHECK(cudaMemcpy(surf_h, surf_d, sizeof(float) * nx * ny, cudaMemcpyDeviceToHost));
  ...
  printf("GPU kernel: %s\n", arrayEqualsMoreOrLess(surf, surf_h, nx * ny) ? "OK" : "KO");
