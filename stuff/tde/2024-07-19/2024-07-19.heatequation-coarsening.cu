...
#define COLS_PER_BLOCK (BLOCKSIZE*COARSENING_FACTOR)
...
__global__ void compute_on_gpu(float *surf, float *surf1, int nx, int ny) {
  ...
  int start_i = blockIdx.x * COLS_PER_BLOCK;
  int end_i = (blockIdx.x+1) * COLS_PER_BLOCK;
  int j = blockIdx.y * blockDim.y + threadIdx.y;

  if(j < ny) {
    for(int i = start_i + threadIdx.x; i < nx && i < end_i; i+=blockDim.x){
...
int main(int argc, char **argv) {
  ...
  CHECK(cudaMalloc(&surf_gpu, nx * ny * sizeof(float)));
  CHECK(cudaMalloc(&surf1_gpu, nx * ny * sizeof(float)));
  ...
  dim3 blocksPerGrid((nx-1)/COLS_PER_BLOCK+1, (ny-1)/BLOCKSIZE+1, 1);
