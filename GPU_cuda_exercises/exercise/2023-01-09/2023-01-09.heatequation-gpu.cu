...
__global__ void initialize_gpu(float *surf,
                               int nx,
                               int ny,
                               float radius2)
{
    ...
    if (tx < nx && ty < ny)
    {
        int index = getIndex(tx, ty, ny);
        // Distance of point i, j from the origin
        float ds2 = (tx - nx / 2) * (tx - nx / 2) + (ty - ny / 2) * (ty - ny / 2);
        if (ds2 < radius2)
            surf[index] = 65.0f;
        else
            surf[index] = 5.0f;
...
__global__ void compute_gpu(float *currSurf,
                            float *nextSurf,
                            int nx,
                            int ny)
{
    ...
    if (tx < nx && ty < ny)
    {
        index = getIndex(tx, ty, ny);
        sij = currSurf[index];
        if (tx > 0)
            sim1j = currSurf[getIndex(tx - 1, ty, ny)];
        else
            sim1j = sij;
        if (ty > 0)
            sijm1 = currSurf[getIndex(tx, ty - 1, ny)];
        else
            sijm1 = sij;
        if (tx < nx - 1)
            sip1j = currSurf[getIndex(tx + 1, ty, ny)];
        else
            sip1j = sij;
        if (ty < ny - 1)
            sijp1 = currSurf[getIndex(tx, ty + 1, ny)];
        else
            sijp1 = sij;
...
int main(int argc, char **argv)
{
    ...
    float *d_surf, *d_surf1, *d_temp;
    cudaMalloc(&d_surf, sizeof(float) * nx * ny);
    cudaMalloc(&d_surf1, sizeof(float) * nx * ny);

    dim3 threadsPerBlock(BLOCK_DIM, BLOCK_DIM);
    dim3 blocksPerGrid((nx - 1) / BLOCK_DIM + 1, (ny - 1) / BLOCK_DIM + 1);

    initialize_gpu<<<blocksPerGrid, threadsPerBlock>>>(d_surf, nx, ny, radius2);
    cudaDeviceSynchronize();

    for (n = 0; n < nsteps; n++)
    {
        compute_gpu<<<blocksPerGrid, threadsPerBlock>>>(d_surf, d_surf1, nx, ny);
        cudaDeviceSynchronize();
    ...
    float *gpu_results = (float *)malloc(sizeof(float) * nx * ny);
    cudaMemcpy(gpu_results, d_surf, sizeof(float) * nx * ny, cudaMemcpyDeviceToHost);

    if (nx * ny <= PROBLEMAPICCOLO)
    {
        printf("\n\n");
        print(gpu_results, nx, ny);
    }
    for (i = 0; i < nx; i++)
    {
        for (j = 0; j < ny; j++)
            if (gpu_results[getIndex(i, j, ny)] != surf[getIndex(i, j, ny)])
            {
                printf("GPU and CPU results are NOT equivalent at index(%d,%d)...\n", i, j);
                exit(EXIT_FAILURE);
            }
    }
